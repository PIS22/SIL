...
public class LignePlacementService {
	...
	public LignePlacement save(LignePlacement ligne) {
		...
		/*
		 * Magasin maga =
		 * ger.mag(lp.getPlacement().getCorrespondant().getMagasinier().getNumMAgasinier
		 * ()); if (lp != null && lp.getArticle().isNumSerieArticle()) {
		 * List<PlageNumDispo> plage =
		 * p.premiersNumerosParArticle(lp.article.getCodeArticle(), "CM"); Long qte =
		 * lp.getQuantiteLignePlacement(); if (qte > 0) { long qt =
		 * plage.get(0).getNumFinPlageDispo() - plage.get(0).getNumDebPlageDispo() + 1;
		 * if (qte < qt) { PlageNumDispo pla = plage.get(0); PlageNumArticle pplace =
		 * new PlageNumArticle(); pplace.setLigneAppro(null);
		 * pplace.setLignePlacement(lp); pplace.setLigneRecollement(null);
		 * pplace.setNumDebPlage(pla.getNumDebPlageDispo());
		 * pplace.setNumFinPlage(pla.getNumDebPlageDispo() + qte - 1); if
		 * (pas.save(pplace) != null) { PlageNumDispo npd = new PlageNumDispo();
		 ...
		 * npd.setNumFinPlageDispo(pplace.getNumFinPlage()); npd.setMagasin(maga); if
		 * (p.save(npd) != null) { pla.setNumDebPlageDispo(npd.getNumFinPlage() + 1);
		 * p.save(pla); } } } else { int i = 0; while (qte >= 0) { PlageNumDispo pla =
		 * plage.get(i); qt = pla.getNumFinPlageDispo() - pla.getNumDebPlageDispo() + 1;
		 * PlageNumArticle pplace = new PlageNumArticle(); pplace.setLigneAppro(null);
		 * pplace.setLignePlacement(lp); pplace.setLigneRecollement(null);
		 * pplace.setNumDebPlage(pla.getNumDebPlageDispo());
		 * pplace.setNumFinPlage(pla.getNumDebPlageDispo() + qte - 1); if (qte >= qt) {
		 * if (pas.save(pplace) != null) { pla.setMagasin(maga); p.saveAndFlush(pla); }
		 * } else { if (pas.save(pplace) != null) { PlageNumDispo npd = new
		 * PlageNumDispo(); npd.setExercice(pla.getExercice());
		 * npd.setNumDebPlage(pplace.getNumDebPlage());
		 * npd.setNumDebPlageDispo(pplace.getNumDebPlage());
		 * npd.setNumFinPlage(pplace.getNumFinPlage());
		 * npd.setNumFinPlageDispo(pplace.getNumFinPlage()); npd.setMagasin(maga); if
