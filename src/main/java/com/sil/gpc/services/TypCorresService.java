...
public class TypCorresService {
	...
	public List<TypCorres> getAll() {
		if (typCorresRepository.findAll().size() == 0) {
			typCorresRepository.saveAndFlush(new TypCorres("COE", "Collecteur"));
			typCorresRepository.saveAndFlush(new TypCorres("LIV", "Livreur"));
		}
